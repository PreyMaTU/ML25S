...
def encode_dataset_loan( x: pd.DataFrame, y: pd.DataFrame ):
  if y is not None:
    # Convert classes A-G to numbers
    y = pd.DataFrame( { 'grade': pd.Categorical( y.grade ).codes } )

    # One hot encode the labels while keeping the original column
    y['grade_to_encode']= y['grade']
    y = pd.get_dummies(y, columns=['grade_to_encode'])
